...
import { IncomingInput, FeedbackRatingType } from '@/queries/sendMessageQuery';
import { BackendConfig } from '@/types';
import { getBackendQueries, createBackendConfig } from '@/queries/backendFactory';
...
export type BotProps = {
  chatflowid: string;
  apiHost?: string;
  backendProvider?: 'default' | 'custom';
...
export const Bot = (botProps: BotProps & { class?: string }) => {
  ...
  // Create backend configuration and get the appropriate backend queries
  const getBackendAPI = () => {
    const backendConfig = createBackendConfig(props.backendProvider || 'default', props.apiHost);
    return getBackendQueries(backendConfig);
  };
  ...
  const updateErrorMessage = (errorMessage: string) => {
    setMessages((prevMessages) => {
      const allMessages = [...prevMessages, { message: props.errorMessage || errorMessage, type: 'apiMessage' as messageType }];
      addChatMessage(allMessages as MessageType[]);
      return allMessages as MessageType[];
    });
  };
    ...
    if (fullFileUpload()) {
      ...
      const handleAttachmentUpload = async (file: any) => {
        try {
          ...
          const { data, error } = await getBackendAPI().createAttachmentWithFormData({
      ...
      const uploadResults = await Promise.all(filesWithFullUploadType.map(handleAttachmentUpload));

      if (!uploadResults || uploadResults.some((result) => !result)) {
        throw new Error('Unable to upload documents');
      } else {
        const extractedData = uploadResults.filter(Boolean);
        for (const extractedFileData of extractedData) {
          if (typeof extractedFileData !== 'object' || extractedFileData === null) continue;
          const content = (extractedFileData as any).content;
          const fileName = (extractedFileData as any).name;
      ...
      const handleFileUpload = async (file: any) => {
        try {
          ...
          const { data, error } = await getBackendAPI().upsertVectorStoreWithFormData({
            chatflowid: props.chatflowid,
            apiHost: props.apiHost,
            formData,
            onRequest: props.onRequest,
      ...
      const uploadResults = await Promise.all(filesWithRAGUploadType.map(handleFileUpload));
  ...
  const handleSubmit = async (value: string, action?: IAction | undefined | null) => {
    ...
    if (isChatFlowAvailableToStream()) {
      ...
      const { data, error } = await getBackendAPI().sendMessageQuery({
        chatflowid: props.chatflowid,
        apiHost: props.apiHost,
        body,
        onRequest: props.onRequest,
  ...
  createEffect(async () => {
    ...
    const { data } = await getBackendAPI().isStreamAvailableQuery({
      chatflowid: props.chatflowid,
      apiHost: props.apiHost,
      onRequest: props.onRequest,
    });
    ...
    const { data: configData } = await getBackendAPI().getChatbotConfig({
      chatflowid: props.chatflowid,
      apiHost: props.apiHost,
      onRequest: props.onRequest,
    });
